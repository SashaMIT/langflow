...
async def probe_database(settings_service: SettingsService) -> CheckResult:
    """Open and close a throwaway connection to the application database."""
    database_url = settings_service.settings.database_url
    if not database_url:
        return CheckResult(
            "fail",
            "no database configured",
            "Set LANGFLOW_DATABASE_URL to your production database connection string.",
        )
...
async def probe_storage(settings_service: SettingsService) -> CheckResult:
    ...
    """
    storage_type = (settings_service.settings.storage_type or "local").lower()
...
async def probe_pgvector(_settings_service: SettingsService) -> CheckResult:
    ...
    def _probe() -> None:
        ...
        engine = sa.create_engine(_normalize_driver(dsn))
        try:
            with engine.connect() as conn:
                conn.execute(sa.text("SELECT 1"))
